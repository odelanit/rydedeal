...
//                    }
            ...
            if ($city_id) {
                ...
                $rides = $city->rides()->orderByDesc('created_at')->paginate(4);
                return view('browse-jobs')->with([
                    'rides' => $rides,
                ]);
            } else {
        ...
        if (auth()->check()) {
            ...
            if ($user->id == $ride->owner->id) {
                ...
                if ($user->hasRole('admin')) {
                    ...
                    $others_accepted = false;
                    if ($ride->driver_id && $user->id != $ride->driver_id) {
                        $others_accepted = true;
                    }

                    return view('show-job')->with([
                        'ride' => $ride,
                        'applied' => $applied,
                        'others_accepted' => $others_accepted,
                    ]);
                }
            }
        } else {
...
//        $ride->time_to = new \DateTime($data['time_to']);
    ...
    public function complete(Request $request, $id)
    {
        ...
        $ride->save();
        return back()->with('success', 'You checked complete');
    }
